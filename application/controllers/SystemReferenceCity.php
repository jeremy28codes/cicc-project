...
class SystemReferenceCity extends CI_Controller {
    ...
    public function getAll(){
        $result = $this->M_system_reference_cities->getAll();
        exit(json_encode($result));
    }

    public function getBy_ProvinceID(){
        if($this->input->post()){
            $reference_province_id = $this->security->xss_clean($this->input->post('reference_province_id'));
            $result = $this->M_system_reference_cities->getBy_ProvinceID($reference_province_id);
            exit(json_encode($result));
        }
    }
}
